...
Benchmarked against the standard CSVsniffer test suites, ***csv-nose is the most accurate and robust of the dialect sniffers compared**: it ranks first or second on every dataset on both success ratio and F1, leads outright on POLLOCK, W3C-CSVW, and CSV Wrangling, and is the **only tool that never errors on a single file**.[^bench] This implementation of the Table Uniformity Method achieves 99.55%[^2] accuracy on the [W3C-CSVW test suite](https://github.com/w3c/csvw) by:
